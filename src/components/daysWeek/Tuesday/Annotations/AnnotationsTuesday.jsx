...
export default function AnnotationsTuesday(){

    const [anotacaoTerca,setAnotacaoTerca] = useState([])

    useEffect(() => {
        async function lerAnotacoesTerca(){
            const response = await api.get('/terca/anotacoes?tipoTarefa=anotacoes')
            const data = response.data

            setAnotacaoTerca(data)
        }
    ...
    async function excluirAnotacoes(id){
        ...
        if(confirmar){
            await api.delete(`/terca/anotacoes/${id}`)
            .then(() => {
